...
export default function DashboardPage() {
  ...
  const { forms, formsLoading, loadForms, createForm, deleteForm } = useFormStore();
    ...
    <MainLayout>
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-logo font-bold text-dark-50">Your Forms</h1>
        ...
        {!user ? (
          <div className="rounded-2xl border border-dark-600 bg-dark-800 p-12 text-center space-y-4 shadow-sm">
            ...
            <p className="text-dark-200 text-lg">Sign in to view and manage your forms.</p>
            <Button variant="primary" size="md" onClick={() => setAuthModalOpen(true, "signin")}>
              Sign In
            </Button>
          </div>
        ) : formsLoading ? (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="rounded-2xl border border-dark-600 bg-dark-800 p-6 animate-pulse">
          ...
          <div className="rounded-2xl border-2 border-dashed border-dark-500 bg-dark-800 p-12 text-center space-y-4">
            ...
            <p className="text-dark-200 text-lg">No forms yet. Create your first one!</p>
            <Button
              variant="primary"
              size="md"
              icon={PlusIcon}
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setDeleteConfirmId(deleteConfirmId === form.id ? null : form.id);
                    }}
                  />
                </div>
                ...
                {deleteConfirmId === form.id && (
                  <div className="mb-3 rounded-lg bg-red-500/10 border border-red-500/20 p-3 animate-slideDown">
                    <p className="text-xs text-red-400 mb-2">Delete this form?</p>
                ...
                <div className="flex items-center justify-between text-xs text-dark-400">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${form.settings.isPublished ? "bg-green-500/10 text-green-400" : "bg-dark-600 text-dark-300"}`}>
        ...
        <CreateFormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSubmit={handleCreate} />
      </div>
    </MainLayout>
  );
}
